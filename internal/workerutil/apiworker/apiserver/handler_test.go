...
func TestProcessSuccess(t *testing.T) {
	...
	manager := newHandler(mockMetadataStore, mockWorkerStore, Options{
...
func TestProcessSetLogContents(t *testing.T) {
	...
	if callCount := len(mockMetadataStore.SetIndexLogContentsFunc.History()); callCount != 1 {
		t.Errorf("unexpected set index log contents call count. want=%d have=%d", 1, callCount)
	} else if value := mockMetadataStore.SetIndexLogContentsFunc.History()[0].Arg3; value != "test payload" {
